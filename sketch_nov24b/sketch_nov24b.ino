...
int pos; //角度
int middle = 104;//摆正时（黑线在A2处时）舵机转动角度
int max = 124, half_max = 124;//max：黑线在A4处时转动的角度   half_max：黑线在A3处时转动的角度
int min = 28, half_min = 54;//min：黑线在A0处时转动的角度    half_min：黑线在A1处时转动的角度
//A组电机驱动   //Left
int A_PWM = 6; //控制速度   
int A_DIR = 7; //控制方向  
//B组电机驱动   //Right
int B_PWM = 5; //控制速度 
int B_DIR = 4; //控制方向
float Kp = 90 , Ki = 0, Kd = 0; //pid弯道参数参数（仅需调Kp）
float previous_error = 0, error = 0, P = 0, I = 0, D = 0, PID_value = 0; //pid直道参数
